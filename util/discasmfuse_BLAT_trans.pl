...
my $ooc_cmd = "$blat_path $genome_db $transcript_db -q=rna -dots=100 -maxIntron=$MAX_INTRON  -makeOoc=11.ooc $mask_lower /dev/null";
...

{
    ...
    foreach my $transcript_file (@transcript_files) {
        ...
        my $cmd = "$blat_path $genome_db $transcript_file -q=rna -dots=100 "
            . " -maxIntron=$MAX_INTRON -out=pslx -ooc=11.ooc $mask_lower -minScore=$min_score -minIdentity=$min_identity $transcript_file.pslx";
